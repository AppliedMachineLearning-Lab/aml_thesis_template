...
claim you make in your own voice: predicting any price movements of assets in
financial markets is quite a difficult task for machine learning models
\parencite{deusserhillebrand2018cryptocurrencies}.
...
Whenever it is possible, a figure should be \emph{vector} artwork with its
text embedded as text: TikZ, or a PDF exported correctly from whatever drew
it. Such a figure stays sharp at any zoom, prints at the resolution of the
press rather than of your screen, and its labels remain selectable, searchable
and set in the document's own font. A screenshot or a PNG gives up all four:
it is fixed at whatever resolution it was captured, goes soft in print, and its
axis labels are pixels that no one can copy or search. Plotting libraries like
Matplotlib will save a PDF instead of a PNG on request, which is usually a
one-line change.

Not every diagram is worth writing out in TikZ, and for those we can recommend
draw.io,\footnote{\href{https://github.com/jgraph/drawio}{github.com/jgraph/drawio}.}
a what-you-see-is-what-you-get editor that exports to PDF and so keeps
everything above. Drawing a block diagram by dragging boxes is a good deal
faster than positioning nodes by hand, and the result is still vector artwork
...
\begin{figure}
\centering
\input{figures/examples/neural_network.tex}
...
\begin{subfigure}[b]{0.44\textwidth}
  ...
  \begin{minipage}[c][45mm][c]{\linewidth}
    ...
    \adjustbox{max width=\linewidth, max totalheight=45mm}{%
      \input{figures/examples/neuron.tex}}
  \end{minipage}
...
\begin{subfigure}[b]{0.52\textwidth}
  ...
  \begin{minipage}[c][45mm][c]{\linewidth}
    \centering
    \adjustbox{max width=\linewidth, max totalheight=45mm}{%
      \input{figures/examples/rnn.tex}}
  \end{minipage}
